...
// Format coefficient: {b0, b1, b2, a1, a2}
// Equation:
...
// Sampling WBV
// =======================================================
const float FS = FS_WBV;                 // 400 Hz dari wbv_coefficients.h
const uint32_t SAMPLE_PERIOD_US = 2500;  // 1/400 s = 2500 us
...
// =======================================================
// Setup ADXL345 untuk WBV 400 Hz
// =======================================================
bool setupADXL345() {
  Wire.begin(21, 22);       // SDA = GPIO21, SCL = GPIO22
  Wire.setClock(100000);    // deteksi awal 100 kHz
  ...
  // Standby dulu sebelum konfigurasi
  ...
  // BW_RATE:
  // 0x0C = 400 Hz
  if (!writeRegister(adxlAddress, REG_BW_RATE, 0x0C)) {
    return false;
  }
...
void setup() {
  ...
  Serial.println();
  Serial.println("=================================");
  Serial.println("WBV Main Unit ADXL345 + Wd/Wk Filter");
  Serial.println("=================================");
  ...
  Serial.print("Sampling rate filter WBV = ");
  Serial.print(FS);
  Serial.println(" Hz");

  Serial.println("X, Y -> Wd");
  Serial.println("Z    -> Wk");
}
...
void loop() {
  ...
  if (now - lastSampleMicros >= SAMPLE_PERIOD_US) {
    ...
    // Z    -> Wk
    // ===================================================
    float axWd = filterX.process(ax);
    float ayWd = filterY.process(ay);
    float azWk = filterZ.process(az);
    ...
    // Setelah 400 sampel = 1 detik
    if (sampleCount >= 400) {
      float awx = sqrtf(sumX2 / sampleCount);
      float awy = sqrtf(sumY2 / sampleCount);
      float awz = sqrtf(sumZ2 / sampleCount);

      // Total WBV seated:
      // av = sqrt((1.4*awx)^2 + (1.4*awy)^2 + (1.0*awz)^2)
      float av = sqrtf(
        powf(1.4f * awx, 2.0f) +
        powf(1.4f * awy, 2.0f) +
        powf(1.0f * awz, 2.0f)
